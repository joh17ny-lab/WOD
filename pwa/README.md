...
- **Food** — per-day nutrition diary by meal with calorie ring + macro goals,
  food search (Open Food Facts / optional USDA), barcode scan, saved meals,
  water tracking, and micronutrients. **Create Custom Food** lets you enter a
  quantity + unit grouped as **Solid** (g, kg, oz, lb), **Liquid** (mL, L,
  fl oz, cup, tbsp, tsp), or **Count** (piece, serving); nutrition is entered
  per 100 g / 100 mL / piece and auto-scaled to the amount you log.
- **Macro goals** — set daily calories, protein, carbs, fat, and water targets
  in Settings (or auto-calculate from body stats via the calorie calculator).
...
- **More → Import Activity File** — load a `.tcx` / `.gpx` exported by a watch
  or Garmin Connect; the summary (sport, time, distance, calories) is added to
  your workout Log.
...
- **Device sync:** browsers can't read Apple Health or Garmin Connect directly,
  ...
  The native app reads Apple Health directly (incl. Garmin-via-Health).
